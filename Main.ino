...
void Get_Req() {
  ...
  LCDPrint(0,0,"Cau Hinh AP Wifi",0);
  LCDPrint(1,0,"  Kiem Tra Lai  ",0);
  delay(2000);
  ...
  if(sssid.length()>1 && passs.length()>1 && token.length()>1) {
   ...
   LCDPrint(0,0," Khoi Dong Lai  ",0);
   delay(2000);
   LCDPrint(1,0,"   Hoan Thanh   ",0);
   delay(2000);
   ESP.restart();
}
...
void connectSTA() {
      ...
      while (WiFi.status() != WL_CONNECTED) {
          ...
          if(countConnect++  == 20) {
            ...
            LCDPrint(0,0,"Ko Ket Noi Wifi ",0);
            LCDPrint(1,0,"  Kiem Tra Lai  ",0);
            delay(2000);
            LCDPrint(0,0,"Ket Noi WF:ESP32",0);
            LCDPrint(1,0," IP:192.168.4.1 ",0);
            delay(2000);
            break;
          }
      }
      ...
      if(WiFi.status() == WL_CONNECTED) {
        ...
        if(blynkConnect == false) {
            My_LCD.clear();
            LCDPrint(0,0,"Ko Ket Noi BLYNK",0);
            LCDPrint(1,0,"  Kiem Tra Lai  ",0);
            delay(3000);
            switchAPMode(); 
        }
            ...
            My_LCD.clear();
            LCDPrint(0,0,"Da Ket Noi BLYNK ",0);
            delay(100);
...
void LCD1602_Init() {
   My_LCD.begin(16, 2);
   My_LCD.clear();
   LCDPrint(0,0, "    He Thong    ",0);
   LCDPrint(1,0, "Bao Chay Khi Doc",0);
}
...
void printRelayState() {
    if(relay1State == 0)
      LCDPrint(1,0,"RL1:TAT ", 0);
    else
      LCDPrint(1,0,"RL1:BAT" , 0); 
    if(relay2State == 0)
      LCDPrint(1,9,"RL2:TAT " ,0);
    else
      LCDPrint(1,9,"RL2:BAT" , 0);
}
...
void printMQ2() {
    ...
    LCDPrint(0,0,(char*)("Nguong:" + String((int)MQ2_Value) + "ppm ").c_str(),0); 
}
...
void printWindowState(int windowState)
{
    ...
    My_LCD.clear();
    if(windowState == 1)
      LCDPrint(0,0,"     Mo Cua     ",0); 
    else
      LCDPrint(0,0,"    Dong Cua    ",0); 
    delay(1000);
    My_LCD.clear();
    ...
    while(1) {
         if(autoManual == AUTO) {
          ...
          if(readMQ2() > mq2Thresshold && readFireSensor() == SENSOR_FIRE_ON) {
            ...
            My_LCD.clear();
            LCDPrint(0,0, "    CANH BAO    ",0 );
            LCDPrint(1,0, " NGUONG KHI DOC ",0 );
            buzzerON = 1;
            delay(500);
            My_LCD.clear();
            Serial.println("WARNING Fire & Gas detected");
            LCDPrint(0,0, "    CANH BAO    ",0 );
            LCDPrint(1,0, "CO LUA NGUY HIEM",0 );
            delay(500);
          }
      ...
      case BUTTON1_ID :
        ...
        if(modeSetThresHold == 0) {
          ...
          switch(AP_STA_MODE) {
            case AP_MODE:
              vTaskSuspend(TaskMainDisplay_handle);
              LCDPrint(0,0,"Cai Dat AP Wifi ",0);
              delay(2000); 
              LCDPrint(0,0,"Ket Noi WF:ESP32",0);
              LCDPrint(1,0," IP:192.168.4.1 ",0);
              // Switch to AP mode
